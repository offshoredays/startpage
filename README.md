# Bryan's Start Page - v2.4 기능 개선 ✨🔄🛠️🔥🏄
...
- **스마트 추정**: API 키 없이도 바람 속도 기반 서핑 조건 추정
- **5단계 서핑지수**: 낮음/보통/좋음/매우좋음/위험
- **시각적 표시**: 날씨 위젯에 서핑지수 이모지(🏄) 및 색상으로 표시
- **공공데이터포털**: [API 키 발급하기](https://www.data.go.kr/data/15097079/openapi.do)

### 📐 카테고리 자동 크기 조절 (NEW!)
- **동적 높이**: 북마크 개수에 따라 카테고리 높이 자동 조절
- **공간 효율**: 북마크가 적은 카테고리는 작게, 많은 카테고리는 크게 표시
- **깔끔한 레이아웃**: 불필요한 빈 공간 제거
